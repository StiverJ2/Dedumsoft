...
<div class="content">
    ...
    <div class="card" id="inv-maquinaria">
        ...
        <?php if ($legacy): ?>
        <form method="get" action="inventario_maquinaria.php#inv-maquinaria" class="d-flex flex-wrap gap-2 align-items-end">
            <div>
...
$(function() {
    ...
    function buildMaqForm(data) {
        ...
        var ubOpts = [{ value: '', label: '-- Sin ubicacion --' }].concat(ubicacionesCache);
        var tipoOpts = [{ value: '', label: '-- Seleccione tipo --' }].concat(tipoMaquinariaCache);
        return DsCrud.field({
                name: 'nombre',
                label: 'Nombre',
                value: data.nombre,
    ...
    function openMaqDelete(row) {
        DsCrud.confirm('Eliminar maquinaria "' + row.nombre + '"?', function() {
            DsCrud.api('../api/inventario_maquinaria.php', 'DELETE', { id: row.id }, function() {
    ...
    maqTable = $('#maq-table').DataTable({
        ...
        columns: [
            { data: 'id' },
            { data: 'nombre' },
            { data: 'tipo_nombre', render: formatMaqTipo },
            { data: 'estado', render: formatMaqEstado },
            { data: 'ubicacion_nombre', defaultContent: '' },
            {
                data: null,
                orderable: false,
                searchable: false,
    ...
    DsCrud.initLegacyTable('maq-table', {
        ...
        onDelete: function(id) {
            DsCrud.confirm('Eliminar maquinaria #' + id + '?', function() {
                DsCrud.api('../api/inventario_maquinaria.php', 'DELETE', { id: id }, function() {
                    DsCrud.toast('Maquinaria eliminada', 'success');
                    location.reload();
                }, function(e) {
                    DsCrud.toast(e, 'error');
